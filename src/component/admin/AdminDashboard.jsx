...
// import { FiCalendar, FiHeart, FiUsers, FiDollarSign, FiClock } from "react-icons/fi";
...
//         // Calculate total revenue (assuming each appointment has a fee from vet data)
//         const totalRevenue = adminAppointments.reduce((sum, vet) => {
//           const vetInfo = vetsData.find(v => v._id === vet.veterinarian._id);
//           return sum + vet.appointments.reduce((apptSum, appt) => 
//             apptSum + (vetInfo?.fee || 0), 0);
//         }, 0);

//         // Calculate average appointment time (assuming appointments have duration in minutes)
//         const allAppointments = adminAppointments.flatMap(vet => vet.appointments);
//         const avgTime = allAppointments.length > 0 
//           ? allAppointments.reduce((sum, appt) => sum + (appt.duration || 30), 0) / allAppointments.length
//           : 0;

//         // Set statistics
//         const newStats = {
//           totalUsers: users.length,
//           totalAppointments: adminAppointments.reduce((sum, vet) => sum + vet.totalAppointments, 0),
//           totalVets: vetsData.length,
//           totalRevenue,
...
//           .flatMap(vet => vet.appointments.map(appt => ({
//             id: appt._id,
//             type: "appointment",
//             message: `${appt.status} appointment for ${appt.petName} with Dr. ${vet.veterinarian.name}`,
...
//             { 
//               title: "Total Revenue", 
//               count: `$${statistics.totalRevenue.toLocaleString()}`, 
...
//             { 
//               title: "Avg. Time", 
//               count: `${statistics.avgAppointmentTime} min`, 
//               color: "bg-indigo-500", 
...
//                         style={{ height: `${(item.value / Math.max(...appointmentData.map(d => d.value))) * 100}%` }}
//                       ></div>

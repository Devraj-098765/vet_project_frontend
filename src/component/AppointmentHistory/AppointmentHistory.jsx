...
const AppointmentHistory = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      setError("Please log in to view appointment history");
      setLoading(false);
    }
  }, []);
  ...
  const fetchAppointments = async () => {
    try {
      const response = await axiosInstance.get("/bookings/history");
      console.log("API Response:", response.data); // Debug: Check the full response
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-5xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {appointments.map((appt) => {
                console.log("Veterinarian Image:", appt.veterinarianId?.image); // Debug: Check image value
                return (
                  <div
                    key={appt._id}
                    className="bg-white rounded-2xl shadow-xl overflow-hidden border border-green-100 transition-all hover:shadow-2xl"
                        ...
                        <div className="space-y-3 mb-6">
                          <div className="flex items-center">
                            <div className="w-10 h-10 rounded-full overflow-hidden mr-3">
                              <img
                                src={
                                  appt.veterinarianId?.image
                                    ? appt.veterinarianId.image.startsWith("http")
                                      ? appt.veterinarianId.image
                                      : `${import.meta.env.VITE_API_URL}${appt.veterinarianId.image}`
                                    : "https://placehold.co/40x40" // Updated placeholder
                                }
                                alt={appt.veterinarianId?.name || "Doctor"}
                                className="w-full h-full object-cover"
                                onError={(e) => (e.target.src = "https://placehold.co/40x40")} // Fallback
                              />
                            </div>
                            <div>
                              <p className="text-sm text-gray-500 font-medium">Doctor</p>

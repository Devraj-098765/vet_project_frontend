...
const ViewVeterinaryReports = () => {
  ...
  const downloadReportAsPDF = () => {
    const doc = new jsPDF();
    doc.text(`Veterinary Report for ${currentReport.petName}`, 10, 10);
    doc.text(`Date: ${currentReport.createdAt.split('T')[0]}`, 10, 20);
    doc.text(`Diagnosis: ${currentReport.diagnosis}`, 10, 30);
    doc.text(`Treatment: ${currentReport.treatment}`, 10, 40);
    doc.text(`Medications: ${currentReport.medications || 'None'}`, 10, 50);
    doc.text(`Vaccinations: ${currentReport.vaccinations || 'None'}`, 10, 60);
    doc.text(`Recommendations: ${currentReport.recommendations || 'None'}`, 10, 70);
